...
public class SimpleRouter extends RouteBuilder {
	...
	public void configure() throws Exception {
		...
		rest("/users").description("User REST service")
				...
				.get().description("Find all users").outType(UserD[].class)
				...
				.put("/{id}").description("Update a user").type(UserD.class)
		...
		rest("/parse").description("Parse XML service")
				.get().outType(String.class)
				// Description for api-doc
				.responseMessage().code(204).message("Run parse xml files").endResponseMessage()
				.responseMessage().code(404).message("Jopa....").endResponseMessage()
				.to("direct:valid-xml");
